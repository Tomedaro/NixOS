...
report_obsidian_protocol_surface() {
 echo
 echo "===== obsidian protocol surface ====="
 ...
 if [ -n "$stale_refs" ]; then
   echo "WARN active Templater stale protocol references"
   echo "$stale_refs"
 else
 ...
 if [ -n "$unsafe_refs" ]; then
   echo "WARN active Templater unsafe direct-action references"
   echo "$unsafe_refs"
 else
...
list_queue "manual review actions newest" "$AI_DIR/inbox/actions-manual-review" 3 6
